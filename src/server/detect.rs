...
impl PlatformKind {
    ...
    pub fn is_bukkit(self) -> bool {
        matches!(self, Self::Paper | Self::Purpur | Self::Folia | Self::Pufferfish | Self::Spigot)
    }
...
pub fn inspect_server_jar(path: &Path) -> Result<ServerJarInfo> {
    ...
    let version_json: Option<VersionJson> = read_entry(&mut zip, "version.json").and_then(|s| serde_json::from_str(&s).ok());
    ...
    let mc_version = version_json.as_ref().and_then(|v| v.id.as_deref()).and_then(McVersion::parse);
...
fn detect_platform(main_class: &str, versions_list: &str, names: &[String], file_name: &str) -> PlatformKind {
    ...
    let hay = format!(
        ...
        names
            .iter()
            .filter(|n| n.starts_with("META-INF/versions/"))
            .cloned()
            .collect::<Vec<_>>()
            .join("\n")
            .to_ascii_lowercase(),
...
mod tests {
    ...
    fn brand_detection() {
        ...
        assert_eq!(
            detect_platform("io.papermc.paperclip.Main", "abc\t26.2/paper-26.2.jar\t26.2", &[], "x.jar"),
            PlatformKind::Paper
        );
        assert_eq!(
            detect_platform("com.velocitypowered.proxy.Velocity", "", &[], "velocity.jar"),
