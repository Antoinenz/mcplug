...
pub fn render(f: &mut Frame, state: &mut State, area: Rect) {
    ...
    let rows: Vec<Row> = state
        ...
        .map(|e| {
            ...
            let changes = e.items.iter().map(|i| match &i.from { Some(f) => format!("{} {f}→{}", i.name, i.to), None => format!("+{} {}", i.name, i.to) }).collect::<Vec<_>>().join(", ");
            Row::new(vec![
        ...
        .collect();
    let widths = [Constraint::Length(17), Constraint::Length(16), Constraint::Length(9), Constraint::Length(24), Constraint::Min(20)];
    ...
    let detail = state.flow.journal.get(state.flow.journal_selected).map(|e| format!("{}\n{}", e.outcome, e.note.clone().unwrap_or_default())).unwrap_or_default();
    f.render_widget(Paragraph::new(format!("{detail}\nr revert selected transaction   Esc back")).wrap(Wrap { trim: true }).style(super::dim()), foot);
}
