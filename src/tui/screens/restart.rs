...
pub fn render(f: &mut Frame, state: &mut State, area: Rect) {
    ...
    let sel = |i: usize| if state.flow.restart_choice == i { Style::default().add_modifier(Modifier::REVERSED) } else { Style::default() };
    let control = state.current().map(|v| crate::control::for_server(&v.server, state.mcsm.as_deref().cloned(), &state.loaded.secrets).name()).unwrap_or("?");
    let lines = vec![
