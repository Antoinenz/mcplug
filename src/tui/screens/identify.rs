...
pub fn render(f: &mut Frame, state: &mut State, area: Rect) {
    ...
    let Some(u) = v.undecided.iter().find(|u| u.jar.descriptor.as_ref().is_some_and(|d| d.name == p.name)) else {
        return;
    };
    let inner = super::popup(
        f,
        area,
        80,
        (u.candidates.len() as u16 + 4).max(6),
        &format!("identify {} ({})", p.name, u.jar.file),
    );
